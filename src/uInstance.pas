...
const
  	MVis_ServerID = 'MVis_Instance';
  
var
	FIPCServer: TSimpleIPCServer;

   
procedure InitializeUniqueServer;
begin
	//It's the first instance. Init the server
	if FIPCServer = nil then begin
		FIPCServer := TSimpleIPCServer.Create(nil);
		FIPCServer.ServerID := MVis_ServerID;
...
begin
	...
	try
		client.ServerId := MVis_ServerID;
		result := client.ServerRunning;
		if not result then 
			InitializeUniqueServer;
	finally
		client.Free;
	end;
end;
